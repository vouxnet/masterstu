...
export function DailyFactWidget() {
  ...
  return (
    <div className="rounded-3xl glass-panel p-[1px] shadow-xl relative overflow-hidden group">
      ...
      <div className="relative z-10 bg-[#13111C]/90 backdrop-blur-xl rounded-[23px] p-5 h-full flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-display text-base font-bold text-white flex items-center space-x-2.5">
          ...
          <div className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${getCategoryColor(currentFact.category)}`}>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex items-start space-x-3">
                <span className="text-2xl mt-0.5 shrink-0">{currentFact.emoji}</span>
                <div>
                  <h3 className="text-xs sm:text-sm font-bold text-white mb-1 leading-snug line-clamp-2">
                    {currentFact.title}
                  </h3>
                  <p className="text-[11px] text-gray-300 leading-relaxed font-medium line-clamp-3">
                    {currentFact.content}
                  </p>
                </div>
              </div>
              
              <div className="flex items-center justify-between pt-2 pl-9 text-[10.5px]">
                {currentFact.source ? (
                  <span className="text-gray-400 italic flex items-center space-x-1.5 font-medium truncate max-w-[180px]">
